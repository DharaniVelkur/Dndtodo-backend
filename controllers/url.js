    ...
    if (finduser) {
      ...
      if (setusertoken) {
        const mailOptions = {
          ...
          text: `This link is valid for 2 minutes http://https://dndtodo.vercel.app/forgotpassword/${finduser._id}/${setusertoken.verifytoken}`,
        };
...
async function deletetask(req, res) {
  ...
  try {
    if (finduser) {
      finduser.tasks=await finduser.tasks.filter((task) => task.id !== id);
      await finduser.save();
      return res.status(200).json(finduser);
    } else {
      return res.status(400).json({ error: "user not exist" });
...
async function edittask(req, res) {
  ...
  try {
    if (finduser) {
      const taskIndex = finduser.tasks.findIndex((task) => task.id === id);
      finduser.tasks[taskIndex] = updatedTask;
      await finduser.save();
      return res.status(200).json(finduser);
    } else {
      return res.status(400).json({error:"User not found"});
...
async function updatetask(req,res) {
  ...
  try {
    if(finduser){
      const taskIndex =finduser.tasks.findIndex((task) => task.id === identity);
      finduser.tasks[taskIndex]=newObj;
      await finduser.save();
      return res.status(200).json(finduser);
   } else {
     return res.status(400).json({error:"User not found"});
